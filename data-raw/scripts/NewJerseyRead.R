...
nj <- bind_rows(raw) %>%
  filter(
    SCHOOL_ID == "999" &
      CATEGORY == "TEACHERS" &
      LEA_NAME != "COUNTY TOTAL" & LEA_NAME != "statewide"
  ) %>%
  select(
    DISTRICT_CODE,
    COUNTY_CODE,
  ...
  mutate(
    state = "NJ",
    DISTRICT_CODE = ifelse(
      nchar(DISTRICT_CODE) == 2,
      paste0(00, DISTRICT_CODE),
      DISTRICT_CODE
    ),
    DISTRICT_CODE = ifelse(
      nchar(DISTRICT_CODE) == 3,
      paste0(0, DISTRICT_CODE),
      DISTRICT_CODE
    ),
  ...
  mutate_at(vars(e1, e2, e3, e4, et),
            function(x)
              as.numeric(gsub("\\*", NA, x))) %>% # * indicates data suppression
  rowwise %>%
  mutate(
    sum = sum(e1, e2, e3, e4, na.rm = T),
    nna = is.na(e1) + is.na(e2) + is.na(e3) + is.na(e4),
    e1 = if_else(nna == 1 & is.na(e1), 0, e1),
    e2_impute = if_else(nna == 2 &
                          is.na(e1) & is.na(e2), 1, 0),
    ...
    es = et - sum(e1, e2, e3, e4, na.rm = T)
  ) %>%
  mutate_if(is.numeric, as.integer) %>%
  select(state, year, localid, name, e1, e2, e3, e4, et, es, e2_impute) %>%
