...
# some cases imputed to very small negative values due to percent rounding
dt$p1 <- round(pmax(dt$p1, 0), 2)
dt$p2 <- round(pmax(dt$p2, 0), 2)
dt$p3 <- round(pmax(dt$p3, 0), 2)
dt$p4 <- round(pmax(dt$p4, 0), 2)
